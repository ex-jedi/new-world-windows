...
class ChirpSeo_Auth
{
  ...
  public function lockFeature() {
    if (!$this->activate()) {
      echo '<div class="notification notification-warning">' . PerchLang::get("You're using Chirp Lite. This feature requires a %sChirp Pro license%s.", '<a href="https://grabachirp.com/compare" target="_blank" rel="noopener noreferrer">', '</a>') . '</div>';
      exit;
    }
  }
  ...
  public function check() {
    if (!$this->key) {
      echo '<div class="notification notification-warning">' . PerchLang::get("You're using Chirp Lite. Purchase a %sChirp Pro license%s for even more SEO tools. Enter your Chirp license key in %s Settings %s", '<a href="https://grabachirp.com/compare" target="_blank" rel="noopener noreferrer">', '</a>', '<a href="' . PERCH_LOGINPATH . '/core/settings">', '</a>') . '</div>';
